...
void WriteCycles(FILE* First, FILE* Second, struct CyclesVariables* Generate, int Limit)
{
	...
	for (struct CyclesVariables* p = &Generate[0]; p <= &Generate[Limit - 1]; p += 2)
	{
		...
		if (p + 1 <= &Generate[Limit - 1]) fprintf(Second, "int %s = %s;\n", (p+1)->VarName, (p+1)->VarValue);
		else break;
	}
	fputs(Temp, Second);
...
int main() {
	...
	if (ConfigData[3] == 1) //Перемешивание функций
	{
		...
		for (int* p = &Random[0]; p <= &Random[Count - 1]; p++)
		{
			char FunNum[8] = "Fun";
			char Number[3] = { '\0' };
			_itoa(*p, Number, 10);
